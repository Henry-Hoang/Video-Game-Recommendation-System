...
def data_preprocessing(
    ...
    """
    dirs, _ = os.path.split(train_file)
    input_data = os.path.join(dirs, "inputcsv")
    
    instance_output = _create_instance(input_data)
...
def _data_generating(input_file, train_file, valid_file, test_file, min_sequence=1):
    ...
    for line in f_input:
        ...
        if user_id != last_user_id:
            ...
            history_buy_num = len(item_id_list)
            cate_str = ""
            iid_str = ""
            dt_str = ""
            ...
            if len(dt_str) > 0:
                dt_str = dt_str[:-1]
            if history_buy_num >= min_sequence:
                fo.write(
                    line_split[1]
                    + "\t"
                    + user_id
...
def _get_sampled_data(instance_file, sample_rate):
    ...
    global item2cate
    dirs, _ = os.path.split(instance_file)
    output_file = os.path.join(dirs, "sampled_file")
    columns = ["label", "user_id", "item_id", "timestamp", "cate_id"]
...
def _create_instance(input_file):
    ...
    for line in f_input:
        ...
        if user_things[0] not in user_dict:
            user_dict[user_things[0]] = []
        user_dict[user_things[0]].append((line, float(user_things[2])))
        item_list.append(user_things[1])
        cate_dict[user_things[1]] = user_things[3]
    ...
    for user_behavior in user_dict:
        ...
        for line, _ in sorted_user_behavior:
            user_things = line.split("\t")
            item_id = user_things[1]
            if item_id in cate_dict:
